...
np.set_printoptions(linewidth=1) # 这个参数填的是横向多宽
...
class Model(object):
    ...
    def find_valley(self,frequency=100, walkType='normal'):
        a_vertical= self.coordinate_conversion()
        ...
        for i in range(length - 1):
            ...
            index2 = peak2['index']
            if index1 < index2:
                valley = self.find_minacc(a_vertical, index1, index2)
                valleysteps.append(valley)
            else:
                pass
        valley = self.find_minacc(a_vertical, steps[-1]['index'], len(a_vertical))
        valleysteps.append(valley)
        return valleysteps

    def find_minacc(self, a_vertical, index1, index2):
        max_value = float('inf')  # 预定义一个最大值，不能设置为0
        valley = {'index': max_value, 'acceleration': max_value}
        for i in range(index1, index2):
            if a_vertical[i] < valley['acceleration']:
                valley['acceleration'] = a_vertical[i]
                valley['index'] = i
        return valley
    ...
    # 步长推算
    # Weinberg步长法
    def step_stride_Weinberg(self, max_acceleration,min_acceleration):
        dif = abs(max_acceleration-min_acceleration)
        return np.power(dif, 1/4) * 0.4
    ...
    def pdr_position(self, frequency=100, walkType='normal', offset=0, initPosition=(0, 0)):
        ...
        valleyarray = self.find_valley(frequency=frequency, walkType=walkType)
        position_x = []
        position_y = []
        x = initPosition[0]
        y = initPosition[1]
        ...
        for i, v in enumerate(steps):
            ...
            length = self.step_stride_Weinberg(v['acceleration'], valleyarray[i]['acceleration'])
    ...
    def show_data(self, dataType):
        if dataType=='linear':
            ...
            ax3 = plt.subplot(3,1,3) #第一行第三列图形
            plt.sca(ax1)
            plt.title('x')
            plt.scatter(index,x)
            plt.sca(ax2)
